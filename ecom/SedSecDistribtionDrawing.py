...
%   Detailed explanation goes here
  if (LOG_SEC_SED=='T') %如果需要画含沙量的断面图
        disp('Sediment Section Distribution Drawing ...')
        SED_Sec_Pic_Path=fullfile(OUT_DIRE,'section_distri\sediment'); %含沙量断面图存放路径（指定）
        [s,mess,messid]=mkdir(SED_Sec_Pic_Path); %建立含沙量断面图的存放文件夹（指定）
        ...
        SEC_SED_TIME=str2num(SEC_SED_TIME); %将读入的含沙量断面绘图时刻的字符数组转化为浮点数组
        Num_SEC_SED_TIME=length(SEC_SED_TIME); %总共需要画几个时刻的图像
        SED_contourscale=[-0.1 0 0.1 0.5 1 1.5 2 5]; %需要画的等值线
        ...
        if strcmp(SEC_P_TYPE,'SPECIFY') %如果已在设置文件中指定断面的位置
            for nsec=1:SEC_NUM
                    expression=['SEC_CONTROL_POINT=SEC_CONTROL_POINTS{',num2str(nsec),',1}'];%将断面控制点统一赋值到SEC_CONTROL_POINT中
                    eval(expression);   
            end
        end
        
        for i=1:Num_SEC_SED_TIME %按照需要画图的数量进行循环
            
            disp(['  The ' num2str(SEC_SED_TIME(i)) ' Hours Drawing'])
            
            %------ Elevation Field Distribution Data Reading ,For Check Tideflat Grid ----------
            EPT_Comp_Filename_SN=SEC_SED_TIME(i)/(N_FPT/3600); %按照模式输出的时间间隔，计算需要画图时刻对应的序号
            EPT_Comp_Filename=sprintf('el_field_%06.6d',EPT_Comp_Filename_SN); %模式计算输出的水位文件
            SED_Comp_Filename_SN=SEC_SED_TIME(i)/(N_FPT/3600); %按照模式输出的时间间隔，计算需要画图时刻对应的序号
            ...
            if ~(exist(fullfile(EPT_Comp_Filepath,EPT_Comp_Filename))&&exist(fullfile(SED_Comp_Filepath,SED_Comp_Filename))) %判断水位场文件和含沙量场文件是否都存在，不存在则输出提示信息
                ...
                for i1=1:IM
                    for j1=1:JM
                        if (h(i1,j1)+EPT_Comp_Data(i1,j1)<DMIN||h(i1,j1)<-10) %将潮滩和陆地处的含沙量赋值为nan和干湿网格标记
                ...
                contourf(xr,yr,SED_Comp_Data(:,:,1),SED_contourscale);
                shading flat; 
                ...
                [cc,hh]=contour(xr,yr,SED_Comp_Data(:,:,1),SED_contourscale);
                clabel(cc,hh,'FontSize',10,'FontName','Times new roman');
                set(hh,'EdgeColor',[0.3 0.3 0.3],'linewidth',1) %设置等值线的颜色和粗细
                
                %  				title([num2str(Pic_time(1)) '年' num2str(Pic_time(2)) '月' num2str(Pic_time(3)) '日' num2str(Pic_time(4)) '时 表层含沙量分布'])
                ...
                if strcmp(SEC_P_TYPE,'SPECIFY') %如果断面位置预先指定 
                    ...
                    for nsec=1:SEC_NUM %按照断面数目进行循环
                        ...
                        figure(1),plot(SEC_CONTROL_POINTS(:,1),SEC_CONTROL_POINTS(:,2),'linewidth',1.5,'color','k'); %在图形1中（含沙量表层平面图）中画出断面的位置
                        hold on
                        ...
                        SEC_SED_POSSITION_X=[SEC_SED_POSSITION_X SEC_CONTROL_POINTS(end,1)]; %将最后一个控制点添加到横坐标向量
                        SEC_SED_POSSITION_Y=[SEC_SED_POSSITION_Y SEC_CONTROL_POINTS(end,2)]; %将最后一个控制点添加到纵坐标向量
                        ...
                        sec_el=plain_interp(SEC_SED_POSSITION_X,SEC_SED_POSSITION_Y,EPT_Comp_Data,IM,JM,KB,xr,yr,fsm,h,h1,h2,-SEC_MAX_HIGHT); %对水位进行双线性插值，陆地处用预先设定的输出图像最高高程的相反数定义（-SEC_MAX_HIGHT） 注：考虑到水位与水深的正方向相反
                        sec_s=plain_interp(SEC_SED_POSSITION_X,SEC_SED_POSSITION_Y,SED_Comp_Data,IM,JM,KB,xr,yr,fsm,h,h1,h2,nan); %对含沙量行双线性插值，陆地处用nan表示
                        ...
                        x_sec_s=ones(KB+1,1)*distance; %有效的含沙量空有KB-1个，加上向两端外插的2个数值，所以垂向为7个数据
                        ...
                        sec_s=[sec_s(1,:);sec_s;sec_s(KB-1,:)]; %含沙量等值外插半个网格
                        ...
                        figure(2),clf;
                        sec_sed_map=load ('Colormap\cm_sed_C.dat');
                        ...
                        caxis([-0.1,5]); 
                        hc=colorbar;
                        set(hc,'ylim',[0 5],'FontName','times new roman','FontSize',12);
                        ...
                        figure(2),eval(['print(gcf,''-dpng'',''' fullfile(SED_Sec_Pic_Path,SED_Comp_Filename) '_specify_sec' num2str(nsec) ''');']); %保存断面图像
                        
                        clear SEC_SED_POSSITION_X SEC_SED_POSSITION_Y x_sec_s y_sec_s distance sec_h sec_el sec_s %清除部分变量
                        
                    end
                    figure(1),eval(['print(gcf,''-dpng'',''' fullfile(SED_Sec_Pic_Path,SED_Comp_Filename) '_specify_sec_check'');']); %保存平面图像
