...
'''   
    with open (tfile,'rb') as f:
        ...
        t[ii,:]  = np.array(struct.unpack('>'+np.str(IM*JM*KB) + 'f', data)).reshape(KB,JM,IM) 
        f.read(4)
'''        
...
m.contourf(x,y,data,levels,cmap = cmap, latlon=True)
m.colorbar()
m.contour(x,y,h,deplevels,latlon=True,linewidths = 0.2)
plt_zlevel = 3.
um = umean[0,:]
vm = vmean[0,:]
...
Q = m.quiver(lon[uv>=ubig],lat[uv>=ubig],u_plt[uv>=ubig],v_plt[uv>=ubig],scale=8,latlon=True,color='purple')
qk = plt.quiverkey(Q,0.2,0.5,0.5,'0.5m/s',color='purple') #,fontsize = 6)
Q = m.quiver(lon[uv<ubig],lat[uv<ubig],u_plt[uv<ubig],v_plt[uv<ubig],scale=3,latlon=True,color='black')
