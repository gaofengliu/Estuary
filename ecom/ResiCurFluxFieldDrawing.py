 if (LOG_FIELD_RCUR=='T') %如果需要画余含沙量断面通量图
    %----------读取网格中心坐标和水深数据-------------------------
...
%       break
   ...
   if (FPT_COLOR_TYPE=='C') %按照绘图的颜色类型设置底图的背景色
    ...
    eval(['load LandFiles_XY\prj\yh101.dat']);
        ...
        yh101=yh101/1000;
%----------------------END------------------------
        disp('Residual Current Flux Field Distribution Drawing ...')
        RCur_Flux_Sec_Pic_Path=fullfile(OUT_DIRE,'residual_distri\field\current_flux'); %余含沙量断面通量图存放路径（指定）
        [s,mess,messid]=mkdir(RCur_Flux_Sec_Pic_Path); %建立余含沙量断面图的存放文件夹（指定）
        ...
        RCur_Flux_Comp_Filepath=fullfile(IN_DIRE,'resi_flux'); %模式的余含沙量断面通量计算结果文件路径
        RCur_Flux_Comp_Fileinfo=dir(fullfile(RCur_Flux_Comp_Filepath,'resi_flux_3d_*.out')); %模式计算的含沙量断面通量（包括文件名，修改时间，大小，是否为文件路径）        
        RCur_Flux_contourscale=[-0.1 0 0.1 0.5 1 1.5 2 5]; %需要画的等值线        
        %计算网格边长
        ...
        for i=1:length(RCur_Flux_Comp_Fileinfo) %按照需要画图的数量进行循环
            ...
            RCur_Flux_Comp_Filename=RCur_Flux_Comp_Fileinfo(i).name; %模式计算输出的余含沙量文件  
            eval(['fid_RCur_Flux_Comp = fopen(''' fullfile(RCur_Flux_Comp_Filepath,RCur_Flux_Comp_Filename) ''',''r'');']); %打开余含沙量文件
            RCur_Flux_U_Comp_Data=nan*ones(IM,JM,KB-1);
            RCur_Flux_V_Comp_Data=nan*ones(IM,JM,KB-1);
            Note=fscanf(fid_RCur_Flux_Comp,'%s',7);
            RCur_Flux_B=fscanf(fid_RCur_Flux_Comp,'%f',1);
            Note=fscanf(fid_RCur_Flux_Comp,'%s',2);
            RCur_Flux_E=fscanf(fid_RCur_Flux_Comp,'%f',1);
            Note=fgetl(fid_RCur_Flux_Comp);
            Note=fgetl(fid_RCur_Flux_Comp);
            disp(['  Residual Sediment Section Distribution from ' num2str(RCur_Flux_B) ' to ' num2str(RCur_Flux_E) ' Hour Drawing'])
            
            while(~feof(fid_RCur_Flux_Comp)) %判读是否读到文件尾部
                I=fscanf(fid_RCur_Flux_Comp,'%d',1); %读I
                J=fscanf(fid_RCur_Flux_Comp,'%d',1); %读J
%               Note=fscanf(fid_RCur_Flux_Comp,'%f',2+KB-1); %读X,Y和余含沙量数据
                Note=fscanf(fid_RCur_Flux_Comp,'%f',2); %读X,Y
                for kk=1:KB-1
                    RCur_Flux_U_Comp_Data(I,J,kk)=fscanf(fid_RCur_Flux_Comp,'%f',1); %读X,Y位置U方向的余含沙通量
                    RCur_Flux_V_Comp_Data(I,J,1:kk)=fscanf(fid_RCur_Flux_Comp,'%f',1); %读X,Y位置V方向的余含沙通量
                end
                Note=fgetl(fid_RCur_Flux_Comp); %读掉末尾的回车
            end
            
            fclose(fid_RCur_Flux_Comp); %关闭计算输出的余含沙量文件
            ...
            for i1=1:IM
                for j1=1:JM
                    if isnan(RCur_Flux_U_Comp_Data(i1,j1))==1 %按照fsmadd判断
                        fsm(i1,j1)=0; 
                        REPT_Comp_Data(i1,j1)=-SEC_MAX_HIGHT; 
                        RCur_Flux_U_Comp_Data(i1,j1,:)=NaN;    
                        RCur_Flux_V_Comp_Data(i1,j1,:)=NaN;    
                    end
            ...
            Pic_time_b=datevec(Init_day+RCur_Flux_B/24); %根据模式设置的起始时间计算余含沙量场统计的起始时间
            Pic_time_e=datevec(Init_day+RCur_Flux_E/24); %根据模式设置的起始时间计算余含沙量场统计的结束时间
            

            
      %=================== 绘制余沙场分布图 ======================
      
        NRCUR_LAYER=str2num(RCUR_LAYER); %将读入的含沙量绘图层数的字符数组转化为浮点数组
        Num_RCUR_LAYER=length(NRCUR_LAYER); %总共需要画几个层次的图像
                   
            for k=1:Num_RCUR_LAYER
                close(figure(1));
                disp(['    Layer ' num2str(NRCUR_LAYER(k))])
 ...
 %           contourf(xr,yr,sqrt(RCur_Flux_U_Comp_Data(:,:,NRSED_LAYER(k)).^2+RCur_Flux_V_Comp_Data(:,:,NRSED_LAYER(k)).^2),RCur_Flux_contourscale);
%             shading flat; 
...
%             end            
%            [cc,hh]=contour(xr,yr,sqrt(RCur_Flux_U_Comp_Data(:,:,NRSED_LAYER(k)).^2+RCur_Flux_V_Comp_Data(:,:,NRSED_LAYER(k)).^2),RCur_Flux_contourscale);
%            clabel(cc,hh,'FontSize',10,'FontName','Times new roman');
%            set(hh,'EdgeColor',[0.3 0.3 0.3],'linewidth',1) %设置等值线的颜色和粗细
            
                if (NRCUR_LAYER(k)==1)
                    RCUR_LAYER_NAME='Surface';
                elseif (NRCUR_LAYER(k)==KB-1)
                    RCUR_LAYER_NAME='Bottom';
                elseif (NRCUR_LAYER(k)==2)
                    RCUR_LAYER_NAME='2nd layer';
                elseif (NRCUR_LAYER(k)==3)
                    RCUR_LAYER_NAME='3rd layer';
                else
                    RCUR_LAYER_NAME=[num2str(NRCUR_LAYER(k)) 'th layer'];
                end
      ...
      %     title(['Residual sediment flux during time from ' num2str(Pic_time_b(4)) ':00 ' num2str(Pic_time_b(2),:) ' ' num2str(Pic_time_b(3)) ' ' num2str(Pic_time_b(1)) ' to ' num2str(Pic_time_e(4)) ':00 ' num2str(Pic_time_e(2),:) ' ' num2str(Pic_time_e(3)) ' ' num2str(Pic_time_e(1)) ' ' RSED_LAYER_NAME])
            ...
            Draw_Flux_U=RCur_Flux_U_Comp_Data*(FPT_XMAX-FPT_XMIN)*0.05/VPT_SCALE; %(FPT_XMAX-FPT_XMIN)*0.05/VPT_SCALE表示将x轴的5%的长度对应VPT_SCALE的大小
            Draw_Flux_V=RCur_Flux_V_Comp_Data*(FPT_YMAX-FPT_YMIN)*0.05/VPT_SCALE; %(FPT_YMAX-FPT_YMIN)*0.05/VPT_SCALE表示将y轴的5%的长度对应VPT_SCALE的大小           
%           quiver(xr(1:VPT_INTERVAL:end,1:VPT_INTERVAL:end),yr(1:VPT_INTERVAL:end,1:VPT_INTERVAL:end),Draw_Flux_U(1:VPT_INTERVAL:end,1:VPT_INTERVAL:end,1),Draw_Flux_V(1:VPT_INTERVAL:end,1:VPT_INTERVAL:end,1),0,'k');
            ...
            plotvector(xr(1:VPT_INTERVAL:end,1:VPT_INTERVAL:end),yr(1:VPT_INTERVAL:end,1:VPT_INTERVAL:end),Draw_Flux_U(1:VPT_INTERVAL:end,1:VPT_INTERVAL:end,NRCUR_LAYER(k)),Draw_Flux_V(1:VPT_INTERVAL:end,1:VPT_INTERVAL:end,NRCUR_LAYER(k)),VPT_SCALE,1,'k','line','uv','fix',1);            
            hold on
        ...
        %    fill(land(:,1),land(:,2),bgcolor); 
         ...
         %   fill(nanhuibiantan_weiken(:,1),nanhuibiantan_weiken(:,2),bgcolor);
            ...
            hold on
            text(FPT_XMIN+(FPT_XMAX-FPT_XMIN)*0.05,FPT_YMIN+(FPT_YMAX-FPT_YMIN)*0.075,[num2str(VPT_SCALE) 'm/s'],'FontName','times new roman','FontSize',10); %标注箭头的量值
            hold on
...
%              hold on
            ...
            set(gca,'ytick',fix(linspace(FPT_YMIN,FPT_YMAX,4)));
             ...
             figure(1),eval(['print(gcf,''-dpng'',''' fullfile(RCur_Flux_Sec_Pic_Path,RCur_Flux_Comp_Filename(1:end-4)) '_' RCUR_LAYER_NAME,''');']); %保存平面图像
             figure(1),eval(['saveas(gcf,''' fullfile(RCur_Flux_Sec_Pic_Path,RCur_Flux_Comp_Filename(1:end-4)) '_' RCUR_LAYER_NAME '.eps'',''psc2 '');']);  %保存断面图像
            end
            close(figure(1));
